...
(define (prcs2wgs x y lo0d la0d)
  (define lo0 (degrees->radians lo0d))
  (define la0 (degrees->radians la0d))
  ;input x y
  ;(define x 11573.375)
  ;(define y 22694.980)
  ;直交座標系の原点
  ;(define lo0 (degrees->radians 139.5000))
  ;(define la0 (degrees->radians 36.0000))
  ;他のパラメータ
  (define F 298.257222101)
  (define alfa 6378137)
  (define m0 0.9999)
  (define n (/ 1 (- (* 2 F) 1)))
  ;秒単位
  ...
  ;有象無象の計算
  (define Sphi (* (/ (* m0 alfa) (+ 1 n)) (+ (* (A 0) la0)
                                             (sigma 1 5
                                                    (lambda (j)
                                                      (* (A j) (sin (* 2 j la0))))))))
  ...
  ;経度緯度
  (define la-rad (+ kai (sigma 1 6 (lambda (j) (* (d j) (sin (* 2 j kai)))))))
  (define lo-rad (+ lo0 (atan (/ (sinh nyu) (cos psi)))))
  (define la (radians->degrees la-rad))
  (define lo (radians->degrees lo-rad))
  (list la lo))
